...
for separation in range(resolution, maxSeparation+resolution, resolution):
    ...
    backgroundMedian = np.median(pdistArray[pdistIndices, :].flatten())
    for pdistIndex in np.nditer(pdistIndices):
        finalMatrix[pdistIndex] = np.sign(np.median(pdistArray[pdistIndex, :]) - backgroundMedian)
np.savetxt('median.txt', finalMatrix, delimiter='\t')
