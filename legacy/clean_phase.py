...
minAgreeCounts = 1
intraThreshold = 10e6
minAgreePercentage = 0.9 # for phased inter only or long range intra
            ...
            for j in range(0, i):
                ...
                if math.sqrt(contactData[leftChr][rightChr][i][1] - contactData[leftChr][rightChr][j][1]) + math.sqrt(abs(contactData[leftChr][rightChr][j][4] - contactData[leftChr][rightChr][i][4])) <= sqrtCleanDistance:
                    if contactData[leftChr][rightChr][j][2] == contactData[leftChr][rightChr][i][2] and contactData[leftChr][rightChr][j][5] == contactData[leftChr][rightChr][i][5]:
                        ...
                        disagreeCounts += 1
            #print nearbyCounts
            if agreeCounts >= minAgreeCounts and ((leftChr == rightChr and contactData[leftChr][rightChr][i][2] == contactData[leftChr][rightChr][i][5] and abs(contactData[leftChr][rightChr][i][1]-contactData[leftChr][rightChr][i][4]) <= intraThreshold) or float(agreeCounts)/(agreeCounts+disagreeCounts) >= minAgreePercentage):           
