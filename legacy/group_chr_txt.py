        ...
        if pileupcolumn.pos == inputPos - 1: # find the position of the SNP
            for pileupread in pileupcolumn.pileups:
                if not pileupread.is_del and not pileupread.is_refskip:
                    ...
                    # add to fragments
                    addRead(fragmentData, phaseData, [read.reference_start, read.reference_end], currentHaplotype)
    counter += 1
    if counter % 1000 == 0:
...
for read in samfile.fetch(chrName):
    ...
    if read.is_paired and not read.is_proper_pair:
        continue
    # add to fragments
    addRead(fragmentData, phaseData, [read.reference_start, read.reference_end], -1)
...
for i in range(len(fragmentData)):
    ...
    for j in range(i+1, len(fragmentData)):
        if phaseData[j] == -3:
            continue
        if fragmentData[j][0] >= fragmentData[i][1]: # no overlaps
            break
        if fragmentData[j][0] - fragmentData[i][0] <= fuzzyRange or abs(fragmentData[j][1] - fragmentData[i][1]) <= fuzzyRange:
            # merge by removing j and updating i
            phaseData[i] = mergePhase(phaseData[i], phaseData[j])      
            if fragmentData[j][1] > fragmentData[i][1]:
                fragmentData[i][1] = fragmentData[j][1]
            phaseData[j] = -3
    if i % 10000 == 0:
        sys.stderr.write("merged starts/ends for "+str(i)+"/"+str(len(fragmentData))+" fragments, last one at "+chrName+" "+str(fragmentData[i][0])+"\n")
...
for i in range(len(fragmentData)):
    ...
    for j in range(i+1, len(fragmentData)):
        ...
        if fragmentData[j][0] >= fragmentData[i][1]: # no overlaps
            break
        if fragmentData[i][1] - fragmentData[j][0] >= expectedOverlap - fuzzyRange and fragmentData[i][1] - fragmentData[j][0] <= expectedOverlap + fuzzyRange:
            # merge by removing j and updating i
            phaseData[i] = mergePhase(phaseData[i], phaseData[j])      
            if fragmentData[j][1] > fragmentData[i][1]:
                fragmentData[i][1] = fragmentData[j][1]
            phaseData[j] = -3
    if i % 10000 == 0:
        sys.stderr.write("merged 9 bp for "+str(i)+"/"+str(len(fragmentData))+" fragments, last one at "+chrName+" "+str(fragmentData[i][0])+"\n")

for i in range(len(fragmentData)):
    if phaseData[i] == -3:
        continue
    sys.stdout.write(chrName+'\t'+str(fragmentData[i][0])+'\t'+str(fragmentData[i][1])+'\t'+str(phaseData[i])+'\n')