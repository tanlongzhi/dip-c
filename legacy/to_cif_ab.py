...
for inputBedgraphFileLine in inputBedgraphFile:
    ...
    # break down each bed interval into discrete beads, assuming each end is divisible by resolution
    for inputLocus in range((int(inputBedgraphFileLineData[1])//resolution)*resolution, (int(inputBedgraphFileLineData[2])//resolution + 1)*resolution, resolution):
        if (inputChr, inputLocus) in bFactorData or int(inputBedgraphFileLineData[3]) == 0: # boundary or unknown: set to 0
            bFactor = 0
        elif int(inputBedgraphFileLineData[3]) > 0: # A
            bFactor = 1
        elif int(inputBedgraphFileLineData[3]) < 0: # B
            bFactor = -1
        bFactorData[(inputChr, inputLocus)] = bFactor
...
for inputPdbLine in inputPdbFile:
    ...
    inputBFactor = 0 # missing: set to 0
