...
parShifts = [] # shift of Y relative to X
for i in range(len(parNames)):
    parShifts.append(parPositions[i][1][0]-parPositions[i][0][0])
# if female: disregard
# if male: same as female if inside the PAR except a coordiante shift if phased as paternal (Y)
# otherwise, non-PAR to non-PAR = unique, non-PAR to phased auto = unique, non-PAR to unphased auto = two possibilities
...
for leftChr in imputeData:
    for rightChr in imputeData[leftChr]:
        for imputeContact in imputeData[leftChr][rightChr]:
            if sex == "m" and leftChr in parNames:
                if imputeContact[2] == 0:
                    imputeContact[0] = yName
                    imputeContact[1] += parShifts[parNames.index(leftChr)]
                elif imputeContact[2] == 1:
                    imputeContact[0] = xName
            if sex == "m" and rightChr in parNames:
                if imputeContact[5] == 0:
                    imputeContact[3] = yName
                    imputeContact[4] += parShifts[parNames.index(rightChr)]
                elif imputeContact[5] == 1:
                    imputeContact[3] = xName
