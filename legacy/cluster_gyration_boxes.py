...
for i in range(numOfLoci):
    clusterData.append([i,i])
finalMatrix = np.zeros([numOfLoci-1, 3], dtype = float)
...
for clusteringStep in range(numOfLoci - 1):
    ...
    finalMatrix[clusteringStep, :] = [clusterData[bestMerge[0]][0], clusterData[bestMerge[1]][1], bestMerge[2]]
    clusterData[bestMerge[0]][1] = clusterData[bestMerge[1]][1] # update the left one
    clusterData[bestMerge[1]][0] = -1 # remove the other one

np.savetxt('link.txt', finalMatrix, delimiter='\t')
