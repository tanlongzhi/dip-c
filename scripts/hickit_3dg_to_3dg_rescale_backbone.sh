...
backbone_unit=$(grep -v "^#" "$input_file" | awk '{chr=$1;loc=$2;x=$3;y=$4;z=$5;if(NR>=2){if(chr==prev_chr){print sqrt((prev_x-x)^2+(prev_y-y)^2+(prev_z-z)^2)}};prev_chr=chr;prev_loc=loc;prev_x=x;prev_y=y;prev_z=z;}' | sort -n | awk '{dist[NR]=$0}END{if(NR%2){print dist[(NR+1)/2]}else{print (dist[(NR/2)]+count[(NR/2)+1])/2.0}}')
